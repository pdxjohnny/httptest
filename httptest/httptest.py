    ...
    def cached(self, key):
        return bool(
            all(
                list(
                    map(
                        lambda needed: os.path.isfile(self.cache_path(key + needed)),
                        [".status", ".headers", ".body"],
                    )
                )
            )
    ...
    def do_forward(self):
        ...
        if self.cached(key):
            ...
            req = urllib.request.Request(
                self.proxied_url(), headers=self.headers, data=data, method=self.command
            )
            try:
...
class HTTPServer(ThreadingHTTPServer):
    ...
    def start_background(self):
        ...
        self.__server = threading.Thread(
            target=self.serve_forever, args=(addr_queue, recv)
        )
        self.__server.start()
